...
main(int argc, char **argv)
{
	...
	if ((argc < 3) || (argc % 2 == 0)) {
		...
		fprintf(stderr,
			" gid is in the form 9c:a1:7d:a9:f2:60:61:92\n");
		fprintf(stderr,
			" [attrn valn] are attribute value pairs of strings\n");
	...
	/*
	 * copy_file("dummy_file", &oid); fetch_file("and_back", &oid);
	 * 
	 * newgid = 0x12345678; err = od_add_gid(&oid, &newgid); assert(err
	 * == 0); 
	 */
	...
	for (i = 3; i < argc;) {
		...
		int             valsize = strlen(val) + 1;	/* include
								 * null
								 * terminator 
								 */
		err = od_set_attr(&oid, key, valsize, val);
		assert(err == 0);
	}

	/*
	 * x = 1; err = od_set_attr(&oid, "foo", sizeof(x), (char *)&x);
	 * assert(err == 0); 
	 */
