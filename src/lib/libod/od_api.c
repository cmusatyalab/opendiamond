...
od_create(obj_id_t *new_obj, groupid_t* gid)
{
    ...
    cl = clnt_create("localhost", MESSAGEPROG, MESSAGEVERS, "tcp");
    assert(cl != NULL);

    cobj.gid = gid;

    cresult = rpc_create_obj_1(&cobj, cl);
    if (cresult == NULL) {
...
od_read(obj_id_t *oid, off_t offset, off_t len, char *buf)
{
    ...
    results = rpc_read_data_1(&rdata, cl);
    if (results == NULL) {
        clnt_perror(cl, "localhost");
        exit(1);
    }
...
od_write(obj_id_t *oid, off_t offset, off_t len, char *buf)
{
    ...
    result = rpc_write_data_1(&wdata, cl);
    if (result == NULL) {
        clnt_perror(cl, "localhost");
        exit(1);
    }
...
od_get_attr(obj_id_t *oid, char *name, int *len, char *buf)
{
    ...
    results = rpc_read_attr_1(&rattr, cl);
    if (results == NULL) {
        clnt_perror(cl, "localhost");
        exit(1);
    }
...
od_set_attr(obj_id_t *oid, char *name, int len, char *buf)
{
    ...
    result = rpc_write_attr_1(&wattr, cl);
    if (result == NULL) {
        clnt_perror(cl, "localhost");
        exit(1);
    }
