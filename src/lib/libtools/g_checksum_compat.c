...
/* Encodes input (u_int32_t) into output (unsigned char). Assumes len is a
 * multiple of 4. */
...
/* Decodes input (unsigned char) into output (u_int32_t). Assumes len is a
 * multiple of 4. */
static void Decode(u_int32_t *output, const unsigned char *input,
		   unsigned int len)
{
	unsigned int i, j;
	for (i = 0, j = 0; j < len; i++, j += 4) {
...
{
	GChecksum *context;
	assert(type == G_CHECKSUM_MD5);

	context = g_slice_new(GChecksum);
...
void g_checksum_update(GChecksum *context, const guchar *input, gssize len)
{
	...
	if ((context->count[0] += ((u_int32_t)len << 3))
	    < ((u_int32_t)len << 3))
		context->count[1]++;
...
void g_checksum_get_digest(GChecksum *context, guint8 *digest, gsize *len)
{
	...
	g_checksum_update (context, PADDING, padLen);

	/* Append length (before padding) */
	g_checksum_update (context, bits, 8);
...
{
    g_slice_free(GChecksum, context);
}
#endif
